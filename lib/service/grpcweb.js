...
function generateGrpcWebPromiseServices(filename, descriptor, exportMap) {
    var serviceDefinitionFilename = getServiceFilename(filename);
    return [
        common_1.createFile(generatePromiseServiceDescriptions(descriptor, exportMap), serviceDefinitionFilename),
    ];
}
exports.generateGrpcWebPromiseServices = generateGrpcWebPromiseServices;
function getServiceFilename(filename) {
    filename = filename.replace('_pb', '');
    return filename + "_grpc_web_pb.d.ts";
}
function generatePromiseServiceDescriptions(fileDescriptor, exportMap) {
    ...
    printer.print("\n    class CanWeDoThis {\n      constructor()\n      ope(): void\n    }\n  ");
    serviceDescriptor.imports.forEach(function (importDescriptor) {
        printer.printLn("import * as " + importDescriptor.namespace + " from \"" + importDescriptor.path + "\";");
    });
    var codePrinter = new CodePrinter_1.CodePrinter(0, printer);
    codePrinter
        .printEmptyLn()
...
function printServicePromisesStub(methodPrinter, service) {
    ...
    printer.printLn("exports." + service.name + "PromiseClient = " + service.name + "PromiseClient;");
}
